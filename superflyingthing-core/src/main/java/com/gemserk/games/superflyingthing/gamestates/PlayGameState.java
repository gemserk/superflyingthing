	...
	class ChallengeMode {
		...
		void loadLevel(EntityTemplates templates, Level level) {
			...
			for (int i = 0; i < level.laserTurrets.size(); i++) {
				...
				entityTemplates.laserTurret(laserTurret.x, laserTurret.y, laserTurret.angle, laserTurret.fireRate, laserTurret.bulletDuration, new Scripts.LaserGunScript(entityTemplates, physicsWorld));
			}
